...
EVENT_OUTPUT = OUTPUT_DIR / "event_sequences.csv"
...
def extract_event_sequences(df, events):

    sequences = []

    for idx in events.index:

        start = idx - PRE_EVENT_WINDOW
        ...
        if len(window) != PRE_EVENT_WINDOW:

            continue

        window["event_index"] = idx

        window["steps_before_event"] = list(
            range(
                -PRE_EVENT_WINDOW,
                0
            )
...
def aggregate_feature_evolution(sequences):
